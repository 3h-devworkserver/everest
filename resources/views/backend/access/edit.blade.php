...
{!! Form::model($user, ['route' => ['admin.access.users.update', $user->id], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PATCH']) !!}
        ...
        <div class="form-group">
            {!! Form::label('fname', trans('validation.attributes.fname'), ['class' => 'col-lg-2 control-label']) !!}
            <div class="col-lg-10">
                {!! Form::text('fname', null, ['class' => 'form-control', 'placeholder' => trans('validation.attributes.fname')]) !!}
            </div>
        </div><!--form control-->

        <div class="form-group">
            {!! Form::label('lname', trans('validation.attributes.lname'), ['class' => 'col-lg-2 control-label']) !!}
            <div class="col-lg-10">
            ...
            <div class="col-lg-3">
                @if (count($roles) > 0)
                    ...
                    @foreach($roles as $role)
                        <input type="radio" value="{{$role->id}}" name="assignees_roles" id="role-{{$role->id}}" @if(!empty($userrole)) @if($role->id == $userrole->id)checked @endif @endif /> <label for="role-{{$role->id}}">{!! $role->name !!}</label> <a href="#" data-role="role_{{$role->id}}" class="show-permissions small">(<span class="show-hide">Show</span> Permissions)</a><br/>
                        ...
                        <div class="col-lg-3">
                            <ul style="margin:0;padding:0;list-style:none;">
                                @foreach ($perm as $p)
                                    ...
                                    <li><input type="checkbox" value="{{$p['id']}}" name="permission_user[]" data-dependencies="{!! $dependencies !!}" id="permission-{{$p['id']}}" @if(in_array($p['id'], $userpermissions)) checked @endif> <label for="permission-{{$p['id']}}" />
