...
class SlidesController extends Controller {
    ...
    public function getImages(){
        $inner_pages = \DB::table('inner_pages')->select('slug')->where('slug', '!=', 'home')->get();
        
        $inner_pages_array = array();
        foreach($inner_pages as $rows){
            $inner_pages_array[$rows->slug] = $rows->slug; 
        }
        return view('backend.gallery.imagesUpload',compact('inner_pages_array'));
    }
    ...
    public function editSlides($id){
        ...
        return view('backend.slides.edit', compact('slide','inner_pages_array'));
    }
    ...
    public function updateSlides($id, Request $request){
     ...
     $teste = $request['image'];
      ...
      if ($slide->update()){
        $captionupdate = DB::table('gallerys')->where('id', $id)->update(['caption'=>$request['caption'],'link'=>$request['link']]); 
