...
class EloquentUserRepository implements UserContract {
	...
	public function getUsers($status = 1, $role) {
		...
		->select('users.*','assigned_roles.user_id','assigned_roles.role_id')
		->where('status', $status)
...
// commented various line -- yojan
	...
	public function update($id, $input) {
		...
		try {
			$user->save();
			// Guide::where('user_id', $user->id)->update(['price' => $input['price'], 'certified' => $certified]);
			// Profile::where('user_id', $user->id)->update(['about' => $input['about'], 'experience' => $input['experience'], 'mGuidingArea' => $input['mGuidingArea'], 'language' => $langConvertoS, 'specilizedarea' => $input['specilizedarea'], 'oGuidingArea' => $ogaConvertoS]);
	...
	public function getAllSlides()
	{
		...
		$slides = Gallery::select('users.username', 'gallerys.path', 'gallerys.id', 'gallerys.caption', 'gallerys.type', 'gallerys.created_at')
	...
	{
		$videos = Video::select('users.username', 'p_videos.video', 'p_videos.id', 'p_videos.pages_id', 'p_videos.created_at')
	...
	{
		// 'gallery.path', for showing images within datatable work later
		$place = GuideArea::select('id','user_name', 'guide_area', 'created_at')
			->get();
	...
	{
		// 'gallery.path', for showing images within datatable work later
		$language = Language::select('id','user_name', 'language', 'created_at')
			->get();
	...
	public function getLicense($status = 1){
		$unapprovedLicense = User::select('users.id','users.email')
			->distinct()
