...
class EloquentAuthenticationRepository implements AuthenticationContract {
	 ...
	 * @throws GeneralException
	 */
	public function travellerLogin($request) {
		...
		if ($this->auth->attempt($request->only('email', 'password'), $request->has('remember')))
		{
			...
			if(access()->hasRole(1)){
				$this->auth->logout();
				throw new GeneralException('Your are admin.And not allowed to login here!!');
			}
            ...
            elseif(access()->hasRole(3)){
				$this->auth->logout();
				throw new GeneralException('Your are not allowed to login here!!');
